...
class Format(object):
    """A format representation that contains:
        ...
        an ODF name (like: 'MS Word 2003 XML') which is the name you must use as
        a filter to call a renderserver or a LibreOffice server
        a mimetype that corresponds to the mimetype of the produced file if you
        ask LibreOffice to convert to the corresponding format
        and a simple flag that indicates if the format is Native or if it is
        produced by calling a LibreOffice filter to convert the native document
...
class Formats(object):
    ...
    def get_format(self, name):
        """an accessor that will return a py3o.formats.Format instance that can
        ...
        :returns: a py3o.formats.Format instance.

        :raises: py3o.formats.UnkownFormatException if the provided name does
